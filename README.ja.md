...
> 明示的な `analyze` / `refresh` コマンドは派生ストアを更新します。
> 読み取りビューは更新せずにストアを読み込みます。`--frozen` を指定すると
...
`analyze` と `refresh` は選択した Codex home を増分処理し、読み取りビューは
選択したストアだけを使用します。進捗と鮮度の診断は stderr に出力され、
...
| `sessions` | 派生ストア | 範囲を限定したセッション情報とカバレッジ | 選択したストアを読み込む。更新しない |
| `inventory` | 派生ストア | 設定済みの対象、使用状況、起動時の推定 | 選択したストアを読み込む。更新しない |
| `overhead` | 派生ストア | 常時読み込まれるコンテキストのコストと残余 | 選択したストアを読み込む。更新しない |
| `usage` | 派生ストア | ツール、Skills、モデル、プロンプト、サブエージェント、対象の使用状況 | 選択したストアを読み込む。更新しない |
| `waste` | 派生ストア | 削除・縮小・適用範囲見直しの候補を順位付け | 選択したストアを読み込む。更新しない |
| `failures` | 派生ストア | 正規化したカテゴリと担当者別の反復失敗 | 選択したストアを読み込む。更新しない |
| `corrections` | 派生ストア | correction レンズの検出結果 | 選択したストアを読み込む。更新しない |
| `rework` | 派生ストア | 従来形式の手戻り検出結果 | 選択したストアを読み込む。更新しない |
| `stuck` | 派生ストア | 範囲を限定した編集・失敗ループと関連パス | 選択したストアを読み込む。更新しない |
| `prompts` | 派生ストア | steer/correct/question/instruct のパターン | 選択したストアを読み込む。更新しない |
| `verification` | 派生ストア | verification レンズの検出結果 | 選択したストアを読み込む。更新しない |
| `knowledge` | 派生ストア | knowledge レンズの検出結果 | 選択したストアを読み込む。更新しない |
| `rediscovery` | 派生ストア | `knowledge` の別名 | 選択したストアを読み込む。更新しない |
| `instructions` | 派生ストア | instruction レンズの検出結果 | 選択したストアを読み込む。更新しない |
| `doctor` | 派生ストア | スコープ別のアクション優先ヘルスサマリー | 選択したストアを読み込む。更新しない |
...
| `optimize --diff` | 派生ストアと対象の指示ファイル | 確度の高い提案差分とスキップ理由 | ストアや対象ファイルを変更しない。レガシーストアは一時移行コピーを使用 |
| `optimize --apply --yes` | 派生ストアと検証済みの指示・文書対象 | 提案を適用し、保持したバックアップと復旧情報を報告 | 検証済み対象だけを変更し、ストアや rollout/state 入力は変更しない |
...
`--diff` と `--print` は助言目的で読み取り専用です。`--print` は diff フラグなしで
...
adapter は通常と zstd 圧縮 rollout JSONL の両方を読み込みます。レポート処理で
元入力を再度開くことはありません。`monitor` は明示的なローカル監視の例外です。
...
`analyze` または `refresh` を実行して選択した派生ストアを更新してから、
読み取りビューで内容を表示します。`--frozen` はストアのみを対象とする境界を
